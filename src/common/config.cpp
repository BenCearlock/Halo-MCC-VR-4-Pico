...
void ConfigSave()
{
    ...
    fprintf(f, "# VRIK arm IK - EXPERIMENTAL, CURRENTLY BROKEN / UNSUPPORTED. 1 = bend\n");
    fprintf(f, "# the arm to your controller (shoulder planted, elbow solved); in\n");
    fprintf(f, "# practice the wrist/arm roots stretch away from the hands, so leave it\n");
    fprintf(f, "# 0. 0 = the supported rigid parent (the whole hand+gun follows the\n");
    fprintf(f, "# controller), which floating hands renders.\n");
    fprintf(f, "# (default %d)\n", d.arm_ik ? 1 : 0);
    fprintf(f, "arm_ik = %d\n\n", g_config.arm_ik ? 1 : 0);
    fprintf(f, "# Floating hands - the SUPPORTED default hand mode. 1 = show only the\n");
    fprintf(f, "# hands and the guns they hold (arms hidden); 0 = full arms. Pure\n");
    fprintf(f, "# render filter, keeps the unsupported arm IK out of sight.\n");
